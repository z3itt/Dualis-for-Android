...
class JobForegroundService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        ensureChannel()
        startInForeground("Waiting for jobs", 0)
    ...
    private suspend fun download(app: com.z3itt.dualis.DualisApplication, track: Track, rawQuery: String) {
        ...
        update(
            ...
            track.copy(
                ...
                coverPath = cover.takeIf { it.exists() }?.absolutePath ?: track.coverPath,
    ...
    private suspend fun separate(app: com.z3itt.dualis.DualisApplication, start: Track) {
        ...
        if (track.coverPath == null) {
            val cover = File(repo.trackDir(track.id), "cover.jpg")
            if (decoder.extractCover(source, cover)) {
                track = track.copy(coverPath = cover.absolutePath)
            }
        }
        var preferNnapi = !InferErrors.shouldFallbackToCpu(track.error.orEmpty())
        var model: LoadedModel? = null
        try {
            model = withContext(Dispatchers.IO) { separator.loadModel(modelFile, spec, preferNnapi) }
            val result = try {
                ...
                if (!InferErrors.shouldFallbackToCpu(msg) || !preferNnapi) throw err
                emit(
                    JobEvent(
                        track.id,
                        "infer",
                        0.1f,
                        InferErrors.cpuFallbackMessage(msg),
                        TrackStatus.SEPARATING.raw(),
                        null,
                    ),
                )
                separator.close(model!!)
                model = withContext(Dispatchers.IO) { separator.loadModel(modelFile, spec, preferNnapi = false) }
                withContext(Dispatchers.Default) {
                    separator.separateFile(model!!, decoded, repo.trackDir(track.id)) { progress, message, eta ->
                        val stage = when {
                            message.contains("decod", true) -> "decode"
    ...
    private suspend fun fail(repo: LibraryRepository, trackId: String, message: String) {
        val track = repo.getTrack(trackId) ?: return
        update(repo, track.copy(status = TrackStatus.ERROR, error = message, updatedAt = now()))
        emit(JobEvent(trackId, "infer", 0f, message, TrackStatus.ERROR.raw(), null))
        startInForeground("Failed: ${track.title}", 0)
    }

    private suspend fun update(repo: LibraryRepository, track: Track) = repo.upsert(track)
